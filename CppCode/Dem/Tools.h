...
static void unitvec (vector <double> & v, int dd, int n) ;
...
static void savecsv (char path[], cv2d & X, cv1d &r, const vector <u_int32_t> & PBCFlags, cv1d & Vmag, cv1d & OmegaMag, cv1d & Z __attribute__((unused))) ;
...
static void vAddScaled (v1d &res , double v, cv1d &a, cv1d &b) {for (uint i=0 ; i<a.size() ; i++) res[i] += v*(a[i]+b[i]) ; }
static void vAddScaled (v1d &res , double v, cv1d &a)          {for (uint i=0 ; i<a.size() ; i++) res[i] += v*a[i] ; }
static void vSubFew (v1d &res , cv1d &a, cv1d &b)                 {for (uint i=0 ; i<a.size() ; i++) res[i] -= (a[i]+b[i]) ; }
static void vSubScaled (v1d &res , double v, cv1d &a)          {for (uint i=0 ; i<a.size() ; i++) res[i] -= v*a[i] ; }
static void vScaledSum (v1d &res , double v, cv1d &a, cv1d &b) {for (uint i=0 ; i<a.size() ; i++) res[i] += v*a[i]+b[i] ; }
...
//=====================================
template <int d>
void Tools<d>::savecsv (char path[], cv2d & X, cv1d &r, const vector <u_int32_t> & PBCFlags, cv1d & Vmag, cv1d & OmegaMag, cv1d & Z __attribute__((unused)))
{
 FILE *out ; int dim ; 
...
//-----------------------------------
template <int d>
void Tools<d>::unitvec (vector <double> & v, int dd, int n)
{
  for (int i=0 ; i<dd ; i++) v[i]=(i==n?1:0) ;
}
//-----------------------------------
template <int d>
