AFRAME.registerComponent('particles', {
    ...
    update: function () {
        ...
        Papa.parse(filename, {
            ...
            complete: function(results) {
                ...
                var R = 0.1;
                if ( data.N > 3 ) { var wristband = document.querySelector('#leftTorus').getObject3D('wristband'); }
                if ( data.N > 5 ) { var wristband1 = document.querySelector('#rightTorus').getObject3D('wristband'); }
                if ( data.N == 1 ) {
                    var geometry = new THREE.CylinderGeometry( 1, 1, 2, Math.pow(2,data.quality), Math.pow(2,data.quality) );
                }
                else {
                    var geometry = new THREE.SphereGeometry( 1, Math.pow(2,data.quality), Math.pow(2,data.quality) );
                }
                var pointsGeometry = new THREE.SphereGeometry( 1, Math.max(Math.pow(2,data.quality-2),4), Math.max(Math.pow(2,data.quality-2),4) );
    ...
    tick: function(t, changed_higher_dim_view) {
        ...
        if ( ( Math.floor(data.time) !== data.prev ) || data.redraw ){
            ...
            if ( data.cache ) { var filename = "http://localhost:8000/Samples/" + data.fname + "dump-"+Math.floor(data.time)+"0000.csv" }
            else { var filename = "http://localhost:8000/Samples/" + data.fname + "dump-"+Math.floor(data.time)+"0000.csv"+"?_="+ (new Date).getTime() }
            Papa.parse(filename, {
                download: true,
                dynamicTyping: true,
                header: true,
                ...
                complete: function(results) {
                    ...
                    for (i = 0; i<spheres.length; i++) {
                        ...
                        if ( data.N == 1 ) { spheres[i].x1 = 0; };
                        if ( data.N < 3 ) { spheres[i].x2 = 0; };
                        object.position.set(spheres[i].x0,spheres[i].x1,spheres[i].x2);
                        if (data.N < 4) {
                        ...
                        if ( data.fname.includes('Submarine') && i==pinky ) { object.visible = false; }
                        else {
                            object.visible = true;
