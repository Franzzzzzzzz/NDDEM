AFRAME.registerComponent('rotate-d4', {
    ...
    init: function () {
        ...
        el.addEventListener('gripdown', function (evt) {
            document.querySelector('#particles').components.particles.data.redraw = true;
            leftHand.getWorldQuaternion(previous_direction);
            previous_torus_rotation_z = document.querySelector("#leftTorus").object3D.rotation.z;
            previous_torus_rotation_y = world[4].cur;
            //console.log(previous_torus_rotation)
            self.data.gripped = true;
        });
        el.addEventListener('gripup', function (evt) {
            document.querySelector('#particles').components.particles.data.redraw = false;
    ...
    tick: function() {
        if ( this.data.gripped ) {
            ...
            // move in D4 by rotations in z
            new_orientation = previous_torus_rotation_y + 2.*diff_angle.y*(world[4].max - world[4].min)/Math.PI;
            if      ( new_orientation < world[4].min )  { new_orientation += (world[4].max - world[4].min); }
            else if ( new_orientation > world[4].max )  { new_orientation -= (world[4].max - world[4].min); }
