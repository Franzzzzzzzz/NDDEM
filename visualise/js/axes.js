...
function make_axes(scene,params,world) {
    if (typeof axesLabels == 'undefined') { // if you haven't already made the axes
        ...
        var arrow_body = new THREE.CylinderGeometry( thickness, thickness, axeslength, Math.pow(2,params.quality), Math.pow(2,params.quality) );
        var arrow_head = new THREE.CylinderGeometry( 0., 2.*thickness, 4*thickness, Math.pow(2,params.quality), Math.pow(2,params.quality) );
        if ( params.N < 3 ) {
    ...
    if (world.ref_dim.c != world.ref_dim.x) {
        ...
        if ( params.N > 3 ) {
            if (world.ref_dim.c < params.N - 1) { world.ref_dim.y = world.ref_dim.c + 1; }
            else { world.ref_dim.y = world.ref_dim.c + 1 - params.N; }
            if (world.ref_dim.c < params.N - 2) { world.ref_dim.z = world.ref_dim.c + 2; }
            else { world.ref_dim.z = world.ref_dim.c + 2 - params.N; }
        }
        else {
            world.ref_dim.y = world.ref_dim.c + 1; world.ref_dim.z = world.ref_dim.c + 2;
        }
    	...
    	loader.load( params.root_dir + 'visualise/node_modules/three/examples/fonts/helvetiker_bold.typeface.json', function ( font ) {
    		var textGeo_x = new THREE.TextBufferGeometry( "x" + world.ref_dim.x, { font: font, size: fontsize, height: fontsize/5., } );
