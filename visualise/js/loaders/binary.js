...
async function load_initial_spheres(params,time) {
                ...
                for (var i = 0; i < nt; i++) {
                    ...
                    for (var j = 0; j < params.num_particles; j++) {
                        ...
                        for (var k = 0; k < params.N+4; k++) {
                            a[i][j][k] = dataview.getFloat32(4 * (k + (params.N+4)*(j + params.num_particles*i)), true);
                        }
                    }
                }
                resolve( a );
            ...
            for (var i = 0; i < nt; i++) {
                ...
                for (var j = 0; j < params.num_particles; j++) {
                    ...
                    for (var k = 0; k < params.N*params.N; k++) {
                        all_rots[i][j][k] = dataview.getFloat32(4 * (k + (params.N*params.N)*(j + params.num_particles*i)), true);
                        }
                    }
                }
    ...
    return await promise.then( function(result) { all_locs = result; return result[time.frame]; });
