...
async function load_world(params,time,world) {
    if (params.fname.substr(-1) === '/') { var fname = params.fname.slice(0,-1); } // remove trailing slash (which we just added) ; //
    if ( params.cache ) { var filename = params.data_dir + "Samples/" + fname }
    else { var filename = params.data_dir + "Samples/" + fname + "?_="+ (new Date).getTime(); }
    ...
    var output = await promise.then( function(result) { return result; });
    return output;
};
...
async function load_initial_spheres(params,time) {
    ...
    var output = await promise.then( function(result) { return result[0]; });
    return output;
};
