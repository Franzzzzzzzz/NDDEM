...
public:
                         ...
                         cv1d &Xj, cv1d &Vj, cv1d &Omegaj, double rj, double mj, cp &Contact) /*, FILE * debug = nullptr*/
  {
    ...
    else
    {
      ...
      vector<double> vel(d, 0);
      vel = Vj;
      loc = Xj;
      loc[0] += P->Boundaries[0][2] * ((ghd & 1) ? -1 : 1);
      loc[1] += (ghd & 1 ? -1 : 1) * P->Boundaries[0][5];
      double additionaldelta = 0;
      if (loc[1] > P->Boundaries[1][1])
      {
        additionaldelta = -P->Boundaries[1][2];
      }
      if (loc[1] < P->Boundaries[1][0])
      {
        additionaldelta = P->Boundaries[1][2];
      }
                                    ...
                                    cv1d &Xj, cv1d &Vj, cv1d &Omegaj, double rj, double mj, cp &Contact)
{
  ...
  bool SAVE_FORCE_COMPONENTS = false; // ADDED BY BENJY!!
