...
void Param::from_json(json &j)
{
    for (auto &v : j.items())
    {
        if (v.key() == "file")
        {
            process_file(v.value());
        }
        else if (v.key() == "savefile")
            save = v.value();
        else if (v.key() == "saveformat")
        {
            ...
            {
                string a = v.value().get<string>();
                saveformat.push_back(a);
            }
        }
        else if (v.key() == "window size")
        {
            ...
            skipT = v.value();
        else if (v.key() == "max time")
        {
            maxT = v.value();
            requiredfieldset[v.key()] = true;
        }
        else if (v.key() == "fields")
        {
        ...
        {
            boxes = v.value().get<decltype(boxes)>();
            requiredfieldset[v.key()] = true;
        }
        else if (v.key() == "boundaries")
        {
            boundaries = v.value().get<decltype(boundaries)>();
            requiredfieldset[v.key()] = true;
        }
        else if (v.key() == "time average")
        {
            if (v.value() == "None")
                timeaverage = AverageType::None;
            else if (v.value() == "Final")
                timeaverage = AverageType::Final;
            else if (v.value() == "Intermediate")
                timeaverage = AverageType::Intermediate;
            else if (v.value() == "Intermediate and Final" || v.value() == "Final and Intermediate")
                timeaverage = AverageType::Both;
            else
                printf("WARN: unknown average type (allowed: 'None', 'Final', 'Intermediate', 'Intermediate and Final'). Case sensitive\n");
        }
        else if (v.key() == "window")
        {
            window = identify_window(v.value().get<string>());
        }
        else if (v.key() == "periodicity")
        {
            periodicity = v.value().get<decltype(periodicity)>();
        }
        else if (v.key() == "density")
        {
            default_density = v.value().get<double>();
        }
        else if (v.key() == "radius")
        {
            default_radius = v.value().get<double>();
        }
        else if (v.key() == "diameter")
        {
            default_radius = v.value().get<double>() / 2.;
        }
        else if (v.key() == "extra fields")
        {
            process_extrafields(v.value());
        }
        else if (v.key() == "dimension")
        {
            dim = v.value();
            requiredfieldset[v.key()] = true;
        }
...
void Param::post_init()
{
    if (files.size() == 0)
    {
        printf("ERR: a file should be defined.\n");
        return;
    }
    if (dim == -1 || !requiredfieldset["dimension"])
        dim = files[0].reader->get_dimension();
    if (!requiredfieldset["window size"])
    {
        printf("ERR: 'window size' is required and cannot be built. \n");
        return;
    }
    if (!requiredfieldset["fields"])
    {
        printf("ERR: 'fields' is required and cannot be built. \n");
        return;
    }
    if (!requiredfieldset["boxes"])
    {
        printf("ERR: the number of boxes in each dimension is required and cannot be built.\n");
        return;
    }
    ...
    if (!requiredfieldset["boundaries"])
        boundaries = files[0].reader->get_bounds();
...
{
    if (windowstr == "Rect3D")
        return Windows::Rect3D;
    else if (windowstr == "Rect3DIntersect")
    {
        printf("====> DEPRECATED: misleading name, use Sphere3DIntersect instead. <=======\n");
        return Windows::Sphere3DIntersect;
    }
    else if (windowstr == "Sphere3DIntersect")
        return Windows::Sphere3DIntersect;
    else if (windowstr == "SphereNDIntersect")
        return Windows::SphereNDIntersect;
    else if (windowstr == "Lucy3D")
        return Windows::Lucy3D;
    else if (windowstr == "Hann3D")
        return Windows::Hann3D;
    else if (windowstr == "RectND")
        return Windows::RectND;
    else if (windowstr == "LucyND")
        return Windows::LucyND;
    else if (windowstr == "LucyND_Periodic")
        return Windows::LucyND_Periodic;
    else if (windowstr == "Lucy3DFancyInt")
        return Windows::Lucy3DFancyInt;
    else
    {
        printf("Unknown windowing function.\n");
        return Windows::Lucy3D;
    }
}
//-------------------------------------
void Param::process_extrafields(json &j)
{
    for (auto &v : j)
    {
        ...
        if (v["type"] == "Particle")
            typ = FieldType::Particle;
        else if (v["type"] == "Contact")
            typ = FieldType::Contact;
        else if (v["type"] == "Fluctuation")
            typ = FieldType::Fluctuation;
        else
        ...
        if (w != v.end())
        {
            extrafields.back().mapping = v["mapping"];
        }
    }
}
//---------------------------------------------------------
void Param::process_file(json &j2)
{
    for (auto j : j2)
    {
        ...
        {
            if (*v == "particles")
                content = FileType::particles;
            else if (*v == "contacts")
                content = FileType::contacts;
            else if (*v == "both")
                content = FileType::both;
            else
                printf("WARN: unknown file content.\n");
        }
        else
        {
            printf("WARN: the key 'content' is recommended when defining a file. Set to 'particles' by default.\n");
        }
        ...
        {
            if (*v == "liggghts")
                format = FileFormat::Liggghts;
            else if (*v == "NDDEM")
                format = FileFormat::NDDEM;
            else if (*v == "mercury_legacy")
                format = FileFormat::MercuryData;
            else if (*v == "mercury_vtu")
                format = FileFormat::MercuryVTU;
            else if (*v == "interactive")
                format = FileFormat::Interactive;
            else
            {
                printf("ERR: unknown file format.\n");
                return;
            }
        }
        else
        {
            printf("ERR: the key 'format' is required when defining a file. \n");
            return;
        }
        ...
        v = j.find("mapping");
        if (v != j.end())
            mapping = v->get<std::map<std::string, std::string>>();
...
{
    D.N = get_num_particles();
    D.Ncf = get_num_contacts();
    ...
    D.mpq.resize(dim * (dim - 1) / 2);
    D.mqp.resize(dim * (dim - 1) / 2);
