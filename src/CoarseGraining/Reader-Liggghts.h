...
public:
    ...
    ~LiggghtsReader()
    {
        ...
        if (filt_in_seekable != nullptr)
        {
            delete (filt_in_seekable);
            filt_in_seekable = nullptr;
        }
        if (file_in != nullptr)
        {
            file_in->close();
    ...
    double *get_data(DataValue datavalue, int dd, std::string name = "")
    {
        ...
        case DataValue::extra_named:
        {
            if (name == "type")
                ...
                printf("Unknown extra: arbitrary liggghts extra fields are not implemented\n");
            return nullptr;
        }
        break;
