...
function set_derived_properties() {
    ...
    // NOTE HACK BELOW: 0.1 OFFSET TO MAKE NUMBERS MORE REASONABLE FOR SMALL SYSTEMS
    let total_volume = params.N * params.particle_volume / (params.initial_packing_fraction - 0.1);
...
function reset_particles() {
    ...
    WALLS.update_top_wall_drained(params, S, scene);
    S.simu_interpret_command("auto location incylinder");
    setup_CG();
    new_load_path();
    started = false;
...
async function init() {
    ...
    gui.add(params, 'drained').name('Drained?').listen().onChange(reset_particles);
...
function setup_NDDEM() {
    ...
    S.simu_interpret_command("boundary 3 AXIALCYLINDER " + String(params.L) + " 2");

    if (params.gravity === true) {
        S.simu_interpret_command("gravity 0 0 " + String(-10) + "0 ".repeat(params.dimension - 3))
    }
    else {
    ...
    // S.simu_interpret_command("set damping 0.0001"); // NOTE: ARTIFICAL DAMPING!!!
...
function update_graph() {
    Plotly.extendTraces('stats', {
        ...
        'y': [[vertical_stress * params.loading_area]],
...
function make_graph() {
    let { data, layout } = LAYOUT.plotly_graph('Vertical Displacement (mm)', 'Vertical Force (N)');
